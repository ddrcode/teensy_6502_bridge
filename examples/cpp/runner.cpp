...
bool Runner::step() {
  ...
  if (!this->phase) {
    ...
    if (this->pins.sync) {
        this->mem->write_byte(0xd012, this->cycle % 255);
    }
    // this->print_state();
...
void Runner::print_state() {
  cout << "[" << dec << this->cycle << "] Addr: " << setfill('0') << setw(4)
       << right << hex << this->pins.addr << " ("
       << addr_to_binary_str(this->pins.addr) << "), Data: " << setfill('0')
       << setw(2) << right << hex << (uint16_t)this->pins.data
       << ", R/W: " << (this->pins.rw ? "R" : "W")
       << ", VP: " << (int)this->pins.vector_pull << ", SYNC "
       << (int)this->pins.sync << ", IRQ " << (int)this->pins.irq << ", NMI "
       << (int)this->pins.nmi << ", RES " << (int)this->pins.reset << ", PHI1O "
       << (int)this->pins.phi1o << ", PHI2O " << (int)this->pins.phi2o << endl;
}
